  ...
  parallax(event) {
    ...
    switch (screen.orientation.type) {
      ...
      case "landscape-primary":
        this.yTilt = Math.round(event.gamma+90) * 0.3; // y axis
        this.xTilt = Math.round(event.beta) * 0.3;  // x axis
        break;
      case "landscape-secondary":
        this.yTilt = Math.round(event.gamma+90) * 0.3; // y axis 
        this.xTilt = Math.round(event.beta+180) * 0.3;  // x axis
        break;
      default:
        permission.style.display = "none";
    }

    this.group.position.x = this.xTilt - this.group.position.x ;
    this.group.position.y = this.yTilt - this.group.position.y;
  }
